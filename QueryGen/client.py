...
class NerClient(object):
    ...
    def get_brands(self):
        ret = self.get_brand_product_bigrams_dict()
        brands = list(json.loads(ret))
        return brands

    def specsBuilder(self,brands,products):
        products = []
        if((len(brands)==1) or (len(products) == 1)):
            specList={}
            ret = []
            if((len(brands) == 0)):
                ret = self.get_spec(product=products[0])
            else:
                ret = self.get_spec(brand=brands[0])        
            featureList = []
            feature = ""
            for i in ret:    
                if(i['category'] == 'Features'):
                    feature = i['field']
                else:
                    feature = i['category'] + " : " + i['field']        
                specList[feature] = {'product':i['product'],'brand':i['brand'],'feature':feature,'value':i['value']}
        else:
            for j in brands:
                #print j
                ret = []
                ret = self.get_spec(brand=j)
                #print ret
                for i in ret:   
                    if(i['category'] == 'Features'):
                        feature = i['field']
